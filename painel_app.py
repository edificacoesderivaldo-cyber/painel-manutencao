    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .portal-card:hover {
            ...
            box-shadow: 0 8px 20px rgba(57, 135, 229, 0.15);
        }
        .portal-card.pay-card {
            border-left: 6px solid #0d6efd;
            background: linear-gradient(135deg, #ffffff 0%, #f0f7ff 100%);
        }
        .portal-card.list-card {
            border-left: 6px solid #1baf7a;
        ...
        .tab-view.active { display: block; animation: fadeIn 0.25s ease-in-out; }
        @keyframes fadeIn {
...
<body>
    <div class="container">
        ...
        <!-- Barra de Navegação Rápida entre Módulos -->
        <div class="nav-bar no-print">
            <button class="nav-btn active" id="btnTabHome" onclick="switchView('home')">
                📊 Painel Inicial (Visão Geral)
            </button>
            <button class="nav-btn" id="btnTabPay" onclick="switchView('pagamentos')">
                💳 Controle de Pagamentos & NF-e
            </button>
            <button class="nav-btn" id="btnTabList" onclick="switchView('chamados')">
                📋 Lista Completa de Chamados
            </button>
        </div>

        <!-- ========================================== -->
        <!-- VIEW 1: PAINEL INICIAL (VISÃO GERAL LIMPA)  -->
        <!-- ========================================== -->
        <div id="viewHome" class="tab-view active">
            <!-- KPIs Gerais -->
            <div class="kpi-section" id="kpis"></div>

            <!-- 2 Botões Grandes de Acesso aos Painéis Detalhados -->
            <div class="portal-cards-grid no-print">
                <div class="portal-card pay-card" onclick="switchView('pagamentos')" title="Acessar o Painel de Liberação de Pagamentos e Emissão de NF-e">
                    <div style="display: flex; align-items: center;">
                        <div class="portal-card-icon">💳</div>
                        <div>
                            <div class="portal-card-title">Controle de Pagamentos & NF-e</div>
                            <div class="portal-card-desc">Conferência por Mês e CNPJ (SESI/SENAI) para autorizar emissão de Nota Fiscal</div>
                        </div>
                    </div>
                    <div class="portal-card-badge" id="portalBadgePay">
                        Acessar Faturamento ➔
                    </div>
                </div>

                <div class="portal-card list-card" onclick="switchView('chamados')" title="Acessar a Lista Completa de Chamados com busca e filtros">
                    <div style="display: flex; align-items: center;">
                        <div class="portal-card-icon">📋</div>
                        <div>
                            <div class="portal-card-title">Lista Completa de Chamados</div>
                            <div class="portal-card-desc">Tabela detalhada com busca por O.S., filtros por Unidade e prazos em aberto</div>
                        </div>
                    </div>
                    <div class="portal-card-badge" id="portalBadgeList">
                        Ver Chamados ➔
                    </div>
                </div>
            </div>
        ...
        <!-- ========================================== -->
        <!-- VIEW 2: PAINEL DE CONTROLE DE PAGAMENTOS   -->
        <!-- ========================================== -->
        <div id="viewPagamentos" class="tab-view">
            <!-- Barra Superior do Módulo com Botão de Retorno -->
            <div class="no-print" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 10px;">
                <button class="btn-return-home" onclick="switchView('home')">
                    ⬅️ Retornar ao Painel Inicial
                </button>
                <div style="font-size: 13px; color: #64748b;">
                    Módulo de Gestão de Faturamento & NF-e
                </div>
            </div>
        ...
        <!-- ========================================== -->
        <!-- VIEW 3: LISTA COMPLETA DE CHAMADOS        -->
        <!-- ========================================== -->
        <div id="viewChamados" class="tab-view">
            <!-- Barra Superior do Módulo com Botão de Retorno -->
            <div class="no-print" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 10px;">
                <button class="btn-return-home" onclick="switchView('home')">
                    ⬅️ Retornar ao Painel Inicial
                ...
                <div style="font-size: 13px; color: #64748b;">
                    Módulo de Consulta Geral de Chamados
                </div>
            </div>
    ...
    <script>
        ...
        function switchView(viewName) {
            ...
            if (viewName === 'home') {
                ...
                // Redimensiona os gráficos Chart.js ao retornar
                setTimeout(() => {
        ...
        function renderPaymentPanel() {
            ...
            // Atualiza badge de prévia no card da Home
            const portalBadge = document.getElementById('portalBadgePay');
            if (portalBadge) {
                portalBadge.innerHTML = `R$ ${fmt(valLiberado)} liberado ➔`;
            }
        ...
        function renderTable() {
            ...
            // Atualiza badge de prévia no card da Home
            const listBadge = document.getElementById('portalBadgeList');
            if (listBadge) {
                listBadge.innerHTML = `${filtered.length} chamados ➔`;
            }
